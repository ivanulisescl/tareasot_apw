/**
 * Visor proyectos - Carga proyectos.json, lista a la izquierda, detalle a la derecha con 3 pestañas.
 */
(function () {
  'use strict';
  ...
  function renderList() {
    ...
    table.className = 'visor-list-table';
    proyectosFiltrados.forEach(function (p) {
      var tr = document.createElement('tr');
      tr.className = 'visor-list-row' + (proyectoSeleccionado && proyectoSeleccionado.idProyecto === p.idProyecto ? ' active' : '');
      var pedido = (p.numeroPedido != null && String(p.numeroPedido).trim() !== '') ? String(p.numeroPedido).trim() : '';
      var nombre = (p.nombreProyecto != null && String(p.nombreProyecto).trim() !== '') ? String(p.nombreProyecto).trim() : '';
      var text = [pedido, nombre].filter(Boolean).join(' ') || 'Sin nombre';
